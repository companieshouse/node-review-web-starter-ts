...
describe("service/SampleServiceOne", () => {
    ...
    it("should serve up a list of items", () => {
        const stub = sinon.stub(instance, "getList").returns(Promise.resolve(fakeData.getList));
        expect(instance.getList()).to.be.fulfilled;
        expect(stub).to.have.been.calledOnce;
    });

    it("should serve up details of a single item", () => {
        const stub = sinon.stub(instance, "getDetails").returns(Promise.resolve(fakeData.getDetails));
        expect(instance.getDetails("abc123")).to.be.fulfilled;
        expect(stub).to.have.been.calledOnce;
    });

    it("should create a record for single item", () => {
        const stub = sinon.stub(instance, "saveItem").returns(Promise.resolve(fakeData.saveItem));
